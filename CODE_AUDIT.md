...
| 6.1 | Open | `src/hooks/useTextToSpeech.ts` | Background receive loop not cancelled on rapid `prepare()` calls | Cancel stale async work; AbortController | — |
...
| **13. Frontend Bugs** | | | | | |
| 13.1 | Open | `src/components/TextInput.tsx` | `onSend(text)` sends untrimmed — guard checks `.trim()` but sends raw | Trim before passing to callback | — |
...
**Totals:** 159 issues — 148 Open, 0 In-progress, 9 Fixed, 2 Skipped
